...
FROM `matricula`, `pack` WHERE `matricula`.`id_pack`= `pack`.`id_pack`  AND `pack`.`id_pack` = ? AND `matricula`.`id_usuario` = ? AND `matricula`.`estado_matricula` = 'A';")) {
		...
		if($grup == 0){
			...
			if ($stmt = $mysqli->prepare("SELECT  `grupos`.`id_grupo`,
				...
				if ( $xop=='si'){
						$valor=correoConfirmacion($email_user, $xnomcomple, $xuser , $nom_pack, $pagoms, $pagomd, $xmoda, $fecha, $xlocal, $iddes );
						if($valor == true){
						}else{
					...
					echo"<h4>No se envio Orden de Pago</h4>";
				}
	...
	if ($xtipo == "INSERTS"){
		...
		if($grup==0 || $est_m == 'x'){
			...
			if($xtipo == 'INSERTS'){
				...
				$sql = "UPDATE `u292000437_bdi`.`usuario` SET `gmail_usuario`='$xgmail@gmail.com', `dni_usuario`='$xdni' WHERE `id_usuario`=$xuser;";
				$stmt = $mysqli->prepare($sql);
				$stmt->execute();
				...
				if ($xlocal =="local"){
						 ...
						 <form method='post' action='o-pago.php'>
								<table class='alt1'>
									  <tr>
										<td  colspan='2' class='op'>
										<div class='opago'><img src='../images/logon.jpg' alt='' width='200' height='100'/></div>
										<h1 ><br>ORDEN DE PAGO<br>$xnomcomple<h1><br><font size='25'>$xuser</font></td>
									...
									<!-- Campo que hace la llamada al script que genera la factura -->
									<input type='hidden' name='generar_factura' value='true'>
									<button type='submit'>DESCARGAR ORDEN DE PAGO</button>
								</form>														
									<ol>
												<b>Recuerde seguir los siguientes pasos para confirmar su matrícula:</b>
												<li>Enviar su boucher de pago escaneado al correo: cursos@innovatrainingperu.com, indicando su código de matrícula y datos personales.</li>
...
/****************************************************/
				...
				$sql= "INSERT INTO `matricula`- PACK";  
			    $stmt->close();
			    bitacora($mysqli, $sql);
				$mysqli->close();
