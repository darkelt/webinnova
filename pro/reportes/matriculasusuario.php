...
if (login_check($mysqli) == true ) {
	if ($_SESSION['permiso'] == 'A'){
		...
		$sql = "SELECT 
				...
				JOIN usuario u
				WHERE m.id_usuario =  u.id_usuario AND u.id_usuario = $id_usuario" ;

		if ($stmt = $mysqli->prepare($sql)) 
		{
			$stmt->execute();
			/* vincular variables a la sentencia preparada */
			$stmt->bind_result($xid,$confir, $xfehora, $xprecio, $xprecio_dolar , $xiduser, $xuser, $xuser_m, $xuser_nom, $xuser_naci, $xuser_email ,$xuser_tel ,$xuser_telopera, $xidgrupo, $xgrupo, $id_pack , $nom_pack,$nom_desc, $descrip_desc, $factor_desc, $localidad);
			while ($stmt->fetch()) {
